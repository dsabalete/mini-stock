...
async function requestProduct(productId: number, quantity: number) {
  try {
    ...
    toast.value =
      'No se pudo enviar la solicitud. Comprueba la disponibilidad.';
  }
  window.setTimeout(() => {
    toast.value = '';
  }, 4000);
...
<template>
  <div v-if="!sessionEmail" class="login-shell">
    ...
    <div class="login-card">
      ...
      <label class="login-label" for="email">Correo corporativo</label><input id="email" v-model="email"
  ...
  <div v-else class="app-shell">
    ...
    <aside class="sidebar">
      ...
      <nav class="main-nav" aria-label="Navegación principal">
        ...
        <button class="nav-item nav-item--active" @click="view = isAdmin ? 'admin' : 'public'">
          <span class="nav-icon">▦</span>
          {{ isAdmin ? 'Panel de control' : 'Catálogo de regalos' }}</button><button v-if="isAdmin" class="nav-item"
          @click="view = 'admin'">
          <span class="nav-icon">♢</span> Solicitudes
          <span class="nav-count nav-count--red">{{
            metrics.pendingRequests
            }}</span></button><button class="nav-item" @click="view = 'public'">
        ...
        <div class="sidebar-status">
          <i></i>
          <div>
            <strong>Operativo</strong><span>Última sincronización<br />hace 2 min</span>
          </div>
        </div>
      </nav>
        ...
        <div class="user-profile">
          ...
          <div>
            <strong>{{
              ...
              }}</strong><span>{{ isAdmin ? 'Administrator' : 'Team member' }}</span>
          </div>
          <button class="more-button" aria-label="Cerrar sesión" @click="sessionEmail = ''">
            ↪
          </button>
        </div>
      </div>
    ...
    <main class="main-content">
      <header class="topbar">
        <div class="breadcrumb">
          <span>Workspace</span><b>/</b><strong>{{
        ...
        <div class="top-actions">
          <span class="role-badge" :class="{ 'role-badge--admin': isAdmin }">{{
            ...
            }}</span><button class="help-button" type="button" aria-label="Abrir ayuda" :aria-expanded="helpOpen"
            aria-controls="help-dialog" @click="helpOpen = true">
            ?
          </button>
        </div>
      </header>
      ...
      <template v-else>
        ...
        <section class="metrics-grid" aria-label="Resumen de inventario">
          <div class="metric-card metric-card--hero">
            <div class="metric-top">
              <span class="metric-label">Unidades en stock</span><span class="metric-badge">+8.4% <span>↗</span></span>
            </div>
            <strong class="metric-number">{{ metrics.totalStock }}</strong>
            <div class="metric-foot">
              <span>unidades físicas</span><span class="mini-bars"><i v-for="bar in [35, 52, 42, 72, 55, 82, 66, 92]"
                  :key="bar" :style="{ height: `${bar}%` }"></i></span>
            </div>
          </div>
          <div class="metric-card">
            <div class="metric-top">
              <span class="metric-label">Solicitudes</span><span class="metric-icon metric-icon--red">!</span>
            </div>
            <strong class="metric-number metric-number--small">{{
              metrics.pendingRequests
              }}</strong>
            <div class="metric-foot">
              <span>pendientes de gestión</span><span class="metric-note metric-note--red">Acción requerida</span>
            </div>
          </div>
          <div class="metric-card">
            <div class="metric-top">
              <span class="metric-label">Artículos bloqueados</span><span
                class="metric-icon metric-icon--amber">▣</span>
            </div>
            <strong class="metric-number metric-number--small">{{
              metrics.lockedItems
              }}</strong>
            <div class="metric-foot">
              <span>reservas en revisión</span><span class="metric-note metric-note--amber">En espera</span>
            </div>
          </div>
          <div class="metric-card">
            <div class="metric-top">
              <span class="metric-label">Valor de inventario</span><span class="metric-icon">€</span>
            </div>
            <strong class="metric-number metric-number--small">{{
              metrics.inventoryValue
              }}</strong>
            <div class="metric-foot">
              <span>precio de coste</span><span class="metric-note">estable</span>
            </div>
          </div>
        </section>
        ...
        <section class="workspace-panel" id="inventario">
          ...
          <div class="filter-row">
            <label class="search-box"><span>⌕</span><input v-model="search" type="search"
                placeholder="Buscar por producto, referencia..." /></label>
            <div class="filter-tabs">
              <button :class="{ active: activeFilter === 'all' }" @click="setFilter('all')">
                Todos <b>12</b></button><button :class="{ active: activeFilter === 'available' }"
                @click="setFilter('available')">
                Disponibles</button><button :class="{ active: activeFilter === 'low' }" @click="setFilter('low')">
                Stock bajo
              </button>
            </div>
          </div>
          ...
          <div class="activity-panel">
            ...
            <div class="activity-list">
              <div v-for="item in recentMovements" :key="item.id" class="activity-row">
                <div class="activity-icon" :class="`activity-icon--${item.type}`">
                  {{ item.type === 'in' ? '↘' : '↗' }}
                </div>
                <div class="activity-copy">
                  <strong>{{ item.title }}</strong><span>{{ item.detail }}</span>
                </div>
                <div class="activity-amount" :class="`activity-amount--${item.type}`">
          ...
          <div class="coverage-panel">
            ...
            <div class="access-rules">
              <div>
                ...
                <p>
                  <strong>Identificación corporativa</strong><small>Solo correos autorizados pueden solicitar</small>
                </p>
                <i>✓</i>
              </div>
              <div>
                <span class="rule-icon">▣</span>
                <p>
                  <strong>Bloqueo automático</strong><small>El artículo queda retenido al solicitar</small>
                </p>
                <i>✓</i>
              </div>
              <div>
                <span class="rule-icon">⌘</span>
                <p>
                  <strong>Gestión centralizada</strong><small>Solo Operations modifica el stock</small>
                </p>
                <i>✓</i>
              </div>
            </div>
