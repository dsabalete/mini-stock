  ...
  <section class="public-catalog">
    <div class="catalog-hero">
      <div>
        ...
        <p>Selecciona una referencia disponible. Tu solicitud quedará bloqueada hasta que Operations la gestione.</p>
      </div>
      <div class="request-protocol"><span class="protocol-line"></span><strong>PROTOCOLO DE PEDIDO</strong><span>01 ·
          Seleccionar</span><span>02 · Solicitar</span><span>03 · Confirmar</span></div>
    </div>
    <div class="catalog-toolbar"><label class="search-box"><span>⌕</span><input v-model="search" type="search"
          placeholder="Buscar por producto o referencia..." /></label>
      <div class="catalog-lines"><button v-for="line in lines" :key="line" :class="{ active: selectedLine === line }"
          @click="selectedLine = line">{{ line === 'ALL' ? 'Todos' : line }}</button></div>
    </div>
      ...
      <article v-for="product in visibleProducts" :key="product.id" class="product-card"
        ...
        <div class="product-card-top"><span class="line-tag" :class="`line-tag--${product.accent}`">{{ product.line
          ...
          @click="zoomedProduct = product"><img :src="`/assets/products/${product.ref}.jpg`" :alt="product.name"
            loading="lazy" /><span class="zoom-hint">⌕ Ampliar</span></button>
        <h2>{{ product.name }}</h2>
    ...
    <Transition name="lightbox">
      <div v-if="zoomedProduct" class="image-lightbox" @click.self="zoomedProduct = null">
        <div class="lightbox-card"><button class="lightbox-close" aria-label="Cerrar imagen"
