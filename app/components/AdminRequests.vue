...
<template>
  <section class="requests-panel">
    ...
    >
      ...
      >
        ...
        <div class="request-info">
          ...
          ><span
            >{{ request.sku }} · {{ request.quantity }} ud. ·
            {{ request.createdAt }}</span
          >
        </div>
        <div class="request-user">
          <span>Solicitado por</span><strong>{{ request.email }}</strong>
        </div>
        <div class="request-actions">
          <button
            class="reject-button"
            @click="emit('manage', request.id, 'rejected')"
          >
            Rechazar</button
          ><button
            class="approve-button"
            @click="emit('manage', request.id, 'approved')"
          >
            Aprobar pedido
          </button>
        </div>
    ...
    <div v-else class="empty-requests">
      <span>✓</span><strong>No hay solicitudes pendientes</strong>
