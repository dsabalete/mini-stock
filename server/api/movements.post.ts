...
export default defineEventHandler(async event => {
  ...
  await db.batch([
    db
      ...
      .bind(next, body.productId),
    db
      .prepare(
        'INSERT INTO movements (title, detail, amount, type, created_at) VALUES (?, ?, ?, ?, ?)'
      )
      .bind(
        `${body.type === 'in' ? 'Entrada' : 'Despacho'} en ${body.location}`,
        body.note || row.name,
        body.quantity,
        body.type,
        now
  ...
  return {
    ...
    movement: {
      title: `${body.type === 'in' ? 'Entrada' : 'Despacho'} en ${body.location}`,
      detail: body.note || row.name,
