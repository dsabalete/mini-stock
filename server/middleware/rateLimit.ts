import { requestRateLimit, authRateLimit, adminRateLimit } from '../utils/rateLimit'
...
export default defineEventHandler(async (event) => {
  ...
  } else if (/^\/api\/requests\/\d+$/.test(event.path) && event.method === 'PATCH') {
    result = await adminRateLimit(event)
  } else {
  ...
  if (!result.allowed) {
    ...
    throw createError({ statusCode: 429, statusMessage: 'Demasiadas peticiones. Intenta de nuevo más tarde.' })
  }
})