...
describe('server/utils/emailVerification', () => {
  ...
  it('should return invalid if domain does not exist', async () => {
    ;(dns.resolveMx as any).mockRejectedValue(new Error('DNS Error'))
    ...
    expect(result.errors).toContain('Domain does not exist or has no DNS records')
  })

  it('should return invalid if SPF record is missing', async () => {
    ;(dns.resolveMx as any).mockResolvedValue([{ exchange: 'mx1.test.com', priority: 10 }])
    ;(dns.resolveTxt as any).mockImplementation((domain: string) => {
  ...
  it('should return valid when all checks pass', async () => {
    ...
    ;(dns.resolveTxt as any).mockImplementation((domain: string) => {
      ...
      if (domain === '_dmarc.test.com') return Promise.resolve(['v=DMARC1; p=reject'])
      return Promise.resolve([])
    })
