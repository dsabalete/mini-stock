...
describe('server/utils/access', () => {
  ...
  it('should throw 503 if domain or aud is missing', async () => {
    ...
    await expect(requireAccess(mockEvent)).rejects.toThrow(
  ...
  it('should throw 401 if token is missing', async () => {
    ...
    await expect(requireAccess(mockEvent)).rejects.toThrow(
      /Falta la autenticación de Cloudflare Access/
    )
  })
  ...
  it('should throw 401 if token is malformed', async () => {
    ...
    await expect(requireAccess(mockEvent)).rejects.toThrow(
      /Token de Cloudflare Access inválido/
    )
  })
  ...
  it('should throw 401 if token claims are invalid (e.g. expired)', async () => {
    ...
    await expect(requireAccess(mockEvent)).rejects.toThrow(
      /Token de Cloudflare Access inválido o caducado/
    )
  })
})
