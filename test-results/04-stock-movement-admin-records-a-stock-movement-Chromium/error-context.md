...
- Name: 04-stock-movement.spec.ts >> admin records a stock movement
- Location: tests/e2e/04-stock-movement.spec.ts:3:1
...
Call log:
  - waiting for getByRole('button', { name: /\+ Registrar movimiento/ })
  ...
  - generic [ref=e3]:
    - complementary [ref=e4]:
      ...
      - navigation "Navegación principal" [ref=e13]:
        ...
        - button "♢ Solicitudes 0" [ref=e17] [cursor=pointer]:
          - generic [ref=e18]: ♢
          - text: Solicitudes
          - generic [ref=e19]: "0"
...
```ts
  ...
  2  | 
  3  | test('admin records a stock movement', async ({ page }) => {
  4  |   await page.goto('/')
...
> 10 |   await page.getByRole('button', { name: /\+ Registrar movimiento/ }).click()
     |                                                                       ^ TimeoutError: locator.click: Timeout 5000ms exceeded.
  11 |   await page.getByRole('button', { name: /SBD/ }).click()
  12 |   await page.getByRole('button', { name: /Salida/ }).click()
  13 |   await page.locator('input[id="quantity"]').fill('5')
  14 |   await page.locator('textarea').fill('Movimiento de prueba')
  15 |   await page.getByRole('button', { name: /Guardar movimiento/ }).click()
  16 | 
  17 |   await expect(page.locator('.toast-message')).toContainText(
  18 |     'Movimiento registrado exitosamente.'
  19 |   )
  20 | })
  21 | 
```